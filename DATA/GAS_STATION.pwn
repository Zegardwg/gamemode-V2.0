...
CMD:fill(playerid, params[])
{
	if(!IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
		return ShowNotifError(playerid, "You must driver a vehicle engine.", 10000);
	
	new vehid = GetPlayerVehicleID(playerid);
	if(!IsEngineVehicle(vehid))
        return ShowNotifError(playerid, "You are not in engine vehicle.", 10000);
	
	if(GetEngineStatus(vehid))
		return ShowNotifError(playerid, "Turn off vehicle engine.", 10000);
			
	if(GetVehicleFuel(vehid) >= 100.0)
		return ShowNotifError(playerid, "This vehicle gas is full.", 10000);
	
	if(pData[playerid][pFill] != -1)
		return ShowNotifError(playerid, "You already filling vehicle. please wait!", 10000);
	...
	foreach(new gsid : GStation)
	{
		if(IsPlayerInRangeOfPoint(playerid, 4.0, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]))
		{
			if(gsData[gsid][gsStock] < 1)
				return ShowNotifError(playerid, "This Gas station is out of stock!", 10000);
...
function Filling(playerid)
{
	...
	foreach(new gsid : GStation)
	{
		if(!IsPlayerInRangeOfPoint(playerid, 4.0, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]) && !IsPlayerInAnyVehicle(playerid) || GetVehicleFuel(GetPlayerVehicleID(playerid)) >= 100.0 || pData[playerid][pMoney] < GStationPrice)
		{
		...
		else
		{
			...
			SetVehicleFuel(GetPlayerVehicleID(playerid), old + 10);
...
StopFilling(playerid)
{
	...
	SuccesMsg(playerid, "Tangki Bensin Sudah Tersisi Penuh!");
...
GetGStationNearest(playerid)
{
	...
	foreach(new id : GStation)
	{
	    ...
	    {
	    	if(GetPlayerDistanceFromPoint(playerid, gsData[id][gsPosX], gsData[id][gsPosY], gsData[id][gsPosZ]) < 1000)
	    	{
     			tmpcount++;
	    	}
		}
