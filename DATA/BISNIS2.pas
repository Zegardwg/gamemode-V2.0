...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData2[bizid][bType2])
    {
        case 1:
        {
            ...
            ShowPlayerDialog(playerid, BISNIS_BUYPROD, DIALOG_STYLE_TABLIST_HEADERS, bData2[bizid][bName2], string, "Buy", "Cancel");
        }
        case 2:
        {
            new MarketHarga[560];
...
Bisnis_Type(bisid)
{
	...
	new query[374];
	mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET intposx='%f', intposy='%f', intposz='%f', intposa='%f', bint='%d' WHERE ID='%d'", bData2[bisid][bIntposX2], bData2[bisid][bIntposY2], bData2[bisid][bIntposZ2], bData2[bisid][bIntposA2], bData2[bisid][bInt2], bisid);
...
CMD:createbiz(playerid, params[])
{
	...
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO bisnis SET ID='%d', owner='%s', ownerid='%d', price='%d', type='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', name='%s'", bid, bData2[bid][bOwner2], bData2[bid][bOwnerID], bData2[bid][bPrice2], bData2[bid][bType2], bData2[bid][bExtposX2], bData2[bid][bExtposY2], bData2[bid][bExtposZ2], bData2[bid][bExtposA2], bData2[bid][bName2]);
...
CMD:editbiz(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET extposx='%f', extposy='%f', extposz='%f', extposa='%f' WHERE ID='%d'", bData2[bid][bExtposX2], bData2[bid][bExtposY2], bData2[bid][bExtposZ2], bData2[bid][bExtposA2], bid);
		mysql_tquery(g_SQL, query);
    ...
    else if(!strcmp(type, "interior", true))
    {
		...
		new query[200];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET bint='%d', intposx='%f', intposy='%f', intposz='%f', intposa='%f' WHERE ID='%d'", bData2[bid][bInt2], bData2[bid][bIntposX2], bData2[bid][bIntposY2], bData2[bid][bIntposZ2], bData2[bid][bIntposA2], bid);
		mysql_tquery(g_SQL, query);
    ...
    else if(!strcmp(type, "locked", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET locked='%d' WHERE ID='%d'", bData2[bid][bLocked2], bid);
		mysql_tquery(g_SQL, query);
    ...
    else if(!strcmp(type, "price", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET price='%d' WHERE ID='%d'", bData2[bid][bPrice2], bid);
		mysql_tquery(g_SQL, query);
	...
	else if(!strcmp(type, "type", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET type='%d' WHERE ID='%d'", bData2[bid][bType2], bid);
		mysql_tquery(g_SQL, query);
	...
	else if(!strcmp(type, "product", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET prod='%d' WHERE ID='%d'", bData2[bid][bProd2], bid);
		mysql_tquery(g_SQL, query);
	...
	else if(!strcmp(type, "money", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET money='%d' WHERE ID='%d'", bData2[bid][bMoney2], bid);
		mysql_tquery(g_SQL, query);
	...
	else if(!strcmp(type, "restock", true))
    {
		...
		if(prod == 0)
		{
			...
			new query[128];
			mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET restock='%d' WHERE ID='%d'", bData2[bid][bRestock2], bid);
			mysql_tquery(g_SQL, query);
	...
	else if(!strcmp(type, "owner", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET owner='%s', ownerid='%d' WHERE ID='%d'", bData2[bid][bOwner2], bData2[bid][bOwnerID], bid);
		mysql_tquery(g_SQL, query);
	...
	else if(!strcmp(type, "point", true))
    {
		...
		new query[128];
		mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET pointx='%f', pointy='%f', pointz='%f' WHERE ID='%d'", bData2[bid][bPointX2], bData2[bid][bPointY2], bData2[bid][bPointZ2], bid);
		mysql_tquery(g_SQL, query);
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "Bali Theater - Bisnis Saya", CMDSString, "Pilih", "Batal");
	return 1;
}
...
CMD:bm(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, BISNIS_MENU, DIALOG_STYLE_LIST, "Bali Theater - Bisnis Menu","Bisnis Info\nGanti Nama\nPendapatan Bisnis\nHarga Product\nRequest Restock","Pilih","Tutup");
    return 1;
}
...
CMD:givebusiness(playerid, params[])
{
	...
	new query[128];
	mysql_format(g_SQL, query, sizeof(query), "UPDATE bisnis SET owner='%s', ownerid='%d', visit='%d' WHERE ID='%d'", bData2[bid][bOwner2], bData2[bid][bOwnerID], bData2[bid][bVisit2], bid);
	mysql_tquery(g_SQL, query);
