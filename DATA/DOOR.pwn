...
Doors_Updatelabel(id)
{
	if(id != -1)
	{
		...
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"{15D4ED}%s\n{FFFFFF}Tekan '"LG_E"F{FFFFFF}' Untuk Masuk", dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicPickup(19133, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint]);
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			...
			{
				new mstr[512];
				format(mstr,sizeof(mstr),"{15D4ED}%s\n{FFFFFF}Tekan {FF0000}ALT {FFFFFF}Untuk Keluar", dData[id][dName]);
				...
				dData[id][dPickupint] = CreateDynamicPickup(19133, 23, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], dData[id][dIntvw], dData[id][dIntint], -1, 7);
			}
			else
			{
				new mstr[512];
				format(mstr,sizeof(mstr),"{15D4ED}%s\n{FFFFFF}Tekan '"LG_E"F{FFFFFF}' Untuk Keluar", dData[id][dName]);
...
function OnDoorsCreated(playerid, id)
{
	Doors_Save(id);
	Doors_Updatelabel(id);
	Servers(playerid, "{15D4ED}Door [%d] berhasil di buat!", id);
	return 1;
}
...
CMD:createdoor(playerid, params[])
{
	...
	format(mstr,sizeof(mstr),"{15D4ED}%s\n{FFFFFF}Tekan '"LG_E"F{FFFFFF}' Untuk Masuk", dData[did][dName]);
	dData[did][dPickupext] = CreateDynamicPickup(19133, 23, dData[did][dExtposX], dData[did][dExtposY], dData[did][dExtposZ], dData[did][dExtvw], dData[did][dExtint]);
...
CMD:gotodoor(playerid, params[])
{
	...
	Servers(playerid, "{15D4ED}Kamu Teleport Ke Pintu ID : {FFFFFF}%d", did);
	return 1;
}
