...
function FurnStore_Load()
{
	...
	printf("*** [Database: Loaded] furnstore data (%d count).", cache_num_rows());
	return 1;
}
...
FurnStore_Create(playerid, price)
{
	...
	if((index = Iter_Free(FurnStore)) != cellmin)
	{
		...
		mysql_tquery(g_SQL, "INSERT INTO `furnstore` (`price`) VALUES (1)", "OnFurnstoreCreated", "d", index);
	...
	if(Iter_Contains(FurnStore, index))
	{
		...
		if(storeData[index][storeOwner])  {
			if (storeData[index][storeSeal]) {
				storeData[index][storeLabel] = CreateDynamic3DTextLabel(sprintf("[ID:%d]\n%s\n"WHITE_E"Owned by: "YELLOW_E"%s\n"WHITE_E"This furniture store is sealed by "RED_E"authority", index, storeData[index][storeName], storeData[index][storeOwnerName]), COLOR_BLUE, storeData[index][storePos][0], storeData[index][storePos][1], storeData[index][storePos][2]+0.5, 10, _, _, 1, 0, 0);
			} else {
				storeData[index][storeLabel] = CreateDynamic3DTextLabel(sprintf("[ID:%d]\n%s\n"WHITE_E"Owned by: "YELLOW_E"%s\n{FFFFFF}Press '{FF0000}ENTER{FFFFFF}' to go inside", index, storeData[index][storeName], storeData[index][storeOwnerName]), COLOR_BLUE, storeData[index][storePos][0], storeData[index][storePos][1], storeData[index][storePos][2]+0.5, 10, _, _, 1, 0, 0);
			}
		}
		else {
			storeData[index][storeLabel] = CreateDynamic3DTextLabel(sprintf("[ID:%d]\n"GREEN_E"This Furniture Store is for sale\n"WHITE_E"Price: "YELLOW_E"%s\n"WHITE_E"Type /buyfurnstore to buy this", index, FormatMoney(storeData[index][storePrice])), COLOR_BLUE, storeData[index][storePos][0], storeData[index][storePos][1], storeData[index][storePos][2]+0.5, 10, _, _, 1, 0, 0);
		}
...
{
	if(pData[playerid][pFurnStore] != -1)
	{
		foreach(new i : FurnStore) if(storeData[i][storeID] == pData[playerid][pFurnStore] && GetPlayerInterior(playerid) == 3 && GetPlayerVirtualWorld(playerid) == (storeData[i][storeID]+1)) {
			return i;
		}
	}
	return -1;
...
FurnStore_IsOwner(playerid, index)
{
	...
	if((Iter_Contains(FurnStore, index) && storeData[index][storeOwner] != 0) && storeData[index][storeOwner] == pData[playerid][pID])
		return 1;
...
FurnStore_IsEmployee(playerid, index)
{
	...
	if(Iter_Contains(FurnStore, index) && storeData[index][storeOwner] != 0) {
		for(new i = 0; i != MAX_EMPLOYE; i++) if(storeData[index][storeEmploye][i] == pData[playerid][pID]) {
			return 1;
		}
	}
	return 0;
...
stock FurnStore_GetCount(playerid) {
	new count = 0;
	foreach (new i : FurnStore) if (storeData[i][storeOwner] == pData[playerid][pID]) count++;
	return count;
}
...
GetDataByIndex(index, type = 0)
{
	...
	getname = mysql_query(g_SQL, sprintf("SELECT `%s` FROM `characters` WHERE `ID`='%d'", !type ? ("Character") : ("LoginDate"), index));
	if(cache_num_rows()) {
		if(!type)
...
ptask productionTimers[1000](playerid)
{
	new 
		keys,ud,lr;
	...
	if(keys == KEY_NO && startProduce[playerid] && IsPlayerInDynamicArea(playerid, production) && GetPVarInt(playerid, "lagiedit") == 2)
	{
		if(++productionCount[playerid] >= 20)
		{
			...
			Servers(playerid, "Production successfull.");
			DeletePVar(playerid, "lagiedit");
		}
		ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
		Info(playerid, "Memproduksi furniture:%d/20",productionCount[playerid], 1000);
	}
}
...
CMD:destroyfurnstore(playerid, params[])
{
	...
	for (new i = 0; i < MAX_FURN_OBJECT; i ++) if(Iter_Contains(FurnObject, i) && FurnStore[i][furnStoreId] == storeData[id][storeID])
	{
		...
		mysql_tquery(g_SQL, sprintf("DELETE FROM `furnobject` WHERE id='%d'", FurnStore[i][furnID]));
	...
	mysql_tquery(g_SQL, sprintf("DELETE FROM `furnstore` WHERE id = '%d'", storeData[id][storeID]));
    ...
    if (response) {
        switch (listitem) {
            case 0: {
                ...
                else Error(playerid, "You don't have any flat");
            }
			case 1: {
                new str[256], count = 0;
                strcat(str, "ID\tAddress\tLocation\n");
                foreach (new i : Houses) if(Player_OwnsHouse(playerid, i) || hData[i][houseBuilder] == pData[playerid][pID]) {
                    strcat(str,sprintf("%d\t%s\t%s\n",i,hData[i][hAddress],GetLocation(hData[i][hExtposX],hData[i][hExtposY],hData[i][hExtposZ])));
                ...
                else Error(playerid, "You don't have any house");
            }
        }
    }
    return 1;
...
Dialog:HouseFurniture(playerid, response, listitem, inputtext[]) {
	if (response) {
		...
		if (storeid != -1) {
            if (!House_IsBuilder(playerid) && !Player_OwnsHouse(playerid, houseid))
                return Error(playerid, "You're not the owner or builder of this house.");

            if(Furniture_GetCount(houseid) >= Furniture_GetMaxItems(houseid))
                return Error(playerid, "You can only have %d furniture items in your house.", Furniture_GetMaxItems(houseid));

            furnid = Furniture_Add(houseid, FurnStore[id][furnName], model, 0.0, 0.0, 0.0);

            if(furnid == cellmin)
                return Error(playerid, "You don't have any house furniture slots left.");
            ...
            SendCustomMessage(playerid, "FURNSTORE", "\"%s"WHITE_E"\" has arrived to your house", FurnStore[id][furnName]);

            FurnStore[id][furnStock]--;
            storeData[storeid][storeVault] += price;
	...
	if (response) {
		...
		if (storeid != -1) {
            ...
            if(FlatFurniture_GetCount(flatid) >= FlatRoom_GetItemsMax(flatid))
                return Error(playerid, "You can only have %d furniture items in your flat.", FlatRoom_GetItemsMax(flatid));

            furnid = FlatFurniture_Add(flatid, FurnStore[id][furnName], model, 0.0, 0.0, 0.0);

            if(furnid == cellmin)
                return Error(playerid, "You don't have any flat furniture slots left.");
            ...
            GivePlayerMoneyEx(playerid, -price);
            SendCustomMessage(playerid, "FURNSTORE", "%s"WHITE_E" has arrived to your flat", FurnStore[id][furnName]);
...
/*Dialog:SelectFurnObject(playerid, response, listitem, inputtext[])
{
		if (response)
		{
		Servers(playerid, "listitem: %d , model: %s", listitem, inputtext);
	}
	return 1;
}
*/
		...
		switch(listitem)
		{
			case 0: {
				if(IsPlayerInDynamicArea(playerid, production))
					return Error(playerid, "Kamu harus berada di luar area kerja terlebih dahulu.");
			...
			case 5: {
				...
				mysql_tquery(g_SQL, sprintf("DELETE FROM `furnobject` WHERE id='%d'", FurnStore[id][furnID]));
...
Dialog:FurnStoreProduct(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if (pData[playerid][pComponent] < component)
			return Error(playerid, "You need %d component(s) to produce furniture.", component);
		...
		Servers(playerid, "Pergilah ke area produksi untuk memulainya.");
	}
...
Dialog:FurnObjectName(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		FurnObject_Refresh(ManageFurnObject[playerid]);
		FurnObject_Save(ManageFurnObject[playerid]);
		Servers(playerid, "Nama telah di ganti menjadi: "GREEN_E"%s", FurnStore[ManageFurnObject[playerid]][furnName]);
	}
	return 1;
}
...
Dialog:FurnObjectPrice(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		FurnObject_Refresh(ManageFurnObject[playerid]);
		FurnObject_Save(ManageFurnObject[playerid]);
		Servers(playerid, "Harga telah di ganti menjadi: "YELLOW_E"%s", FormatMoney((FurnStore[ManageFurnObject[playerid]][furnPrice])));
	}
	return 1;
}
...
Dialog:HireEmployee(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(IsPlayerConnected(userid))
		{
			...
			{
				storeData[index][storeEmploye][i] = pData[playerid][pID];
				FurnStore_Save(index);
				Servers(playerid, "Kamu telah mempekerjakan "YELLOW_E"%s "WHITE_E"ke dalam furniture store milikmu.", ReturnName(userid));
				Servers(playerid, "Kamu telah dipekerjakan oleh "YELLOW_E"%s "WHITE_E"di furniture store miliknya.", ReturnName(playerid));
				printf("Slot: %d", i);
				return 1;
			}
		}
...
Dialog:FiredEmployee(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(response)
		{
			...
			Servers(playerid, "Kamu telah memecat pegawai dari furniture store milikmu.");
...
Dialog:FurnStoreBank(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			...
			case 2: Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, "Balance", "Current balance on this furniture store: %s", "Close", "", FormatMoney(storeData[ManageFurnStore[playerid]][storeVault]));
		}
	}
	return 1;
}
...
Dialog:BankSelect(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		switch(type)
		{
			case 1: GivePlayerMoneyEx(playerid, -strval(inputtext)), storeData[index][storeVault] += strval(inputtext);
			case 2: GivePlayerMoneyEx(playerid, strval(inputtext)), storeData[index][storeVault] -= strval(inputtext);
		}
		Servers(playerid, "Kamu telah %s uang sebesar %s %s furniture store.", (type == 1) ? ("menyimpan") : ("mengambil"), FormatMoney(strval(inputtext)), (type == 1) ? ("untuk") : ("dari"));
		FurnStore_Save(index);
...
Dialog:FurnStoreName(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Servers(playerid, "Nama furniture store di ganti menjadi: %s", storeData[ManageFurnStore[playerid]][storeName]);
...
function editLokasiFurn(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, Float:offx, Float:offy, Float:offz)
{
	if(response == EDIT_RESPONSE_CANCEL)
	{
		if(startProduce[playerid])
		{
			...
			Info(playerid, "Tidak dapat menekan 'ESC' dalam mode ini");
		}
	}
	if(response == EDIT_RESPONSE_FINAL)
	{
		if(startProduce[playerid])
		{
			if(!IsPointInDynamicArea(production, x, y, z))
				return PlayerEditPoint(playerid, 1489.54, 1789.14, 10.90, 0.0, 0.0, 0.0, "editLokasiFurn", produceObject[playerid]), Info(playerid, "Tidak dapat di lakukan di luar area produksi.");

			Servers(playerid, "Mulailah memproduksi furniture dengan menahan tombol "RED_E"'N'");
...
function FurnObject_Load(index)
{
	...
	printf("*** [Database: Loaded] furnobject data (%d count).", cache_num_rows());
	return 1;
}
...
FurnObject_Refresh(index)
{
	if(Iter_Contains(FurnObject, index)) 
	{
		...
		FurnStore[index][furnLabel] = CreateDynamic3DTextLabel(sprintf(""WHITE_E"[id:%d]\n"YELLOW_E"%s\n"WHITE_E"Price: "GREEN_E"%s\n"WHITE_E"Stock: "GREEN_E"%d", index, FurnStore[index][furnName], FormatMoney(FurnStore[index][furnPrice]), FurnStore[index][furnStock]), COLOR_BLUE, FurnStore[index][furnPos][0], FurnStore[index][furnPos][1], FurnStore[index][furnPos][2], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, (FurnStore[index][furnStoreId]+1), 3);

		foreach(new i : Player) if(IsPlayerInRangeOfPoint(i, 5, FurnStore[index][furnPos][0], FurnStore[index][furnPos][1], FurnStore[index][furnPos][2])) {
			Streamer_Update(i);
		}
	}
	return 1;
...
FurnObject_Add(playerid, objectid, index)
{
	...
	if (pData[playerid][pComponent] < 30)
		return Error(playerid, "You need 30 components to create new furniture.");

	new id = Iter_Free(FurnObject),
		Float:x, Float:y, Float:z;

	if (id == cellmin)
		return Error(playerid, "Server has reached the maximum of furniture object!");
	...
	mysql_tquery(g_SQL, "INSERT INTO `furnobject`(`price`) VALUES (0)", "FurnObject_Created", "d", id);
