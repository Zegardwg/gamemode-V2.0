function Flat_Load() {
  ...
  printf("*** [Database: Loaded] flat data loaded (%d count)", cache_num_rows());
  return 1;
}
...
function FlatRoom_Load() {
  ...
  if (cache_num_rows()) {
    for (new i = 0; i < cache_num_rows(); i ++) {
      ...
      mysql_tquery(g_SQL, sprintf("SELECT * FROM `flat_weapon` WHERE `flatroomid` = '%d' ORDER BY `id` DESC LIMIT 3;", FlatRoom[i][flatRoomID]), "OnFlatLoadWeapon", "d", i);

      mysql_tquery(g_SQL, sprintf("SELECT * FROM `flat_storage` WHERE `ID` = '%d';", FlatRoom[i][flatRoomID]), "OnLoadFlatStorage", "d", i);

      mysql_tquery(g_SQL, sprintf("SELECT * FROM `flat_furniture` WHERE `Flatid` = '%d';", FlatRoom[i][flatRoomID]), "OnLoadFlatFurniture", "d", i);
      
      mysql_tquery(g_SQL, sprintf("SELECT * FROM `flat_structure` WHERE `Flatid` = '%d' AND `Static`='1';", FlatRoom[i][flatRoomID]), "OnLoadStaticStructure", "d", i);

      mysql_tquery(g_SQL, sprintf("SELECT * FROM `flat_structure` WHERE `Flatid` = '%d' AND `Static`='0';", FlatRoom[i][flatRoomID]), "OnLoadFlatStructure", "d", i);

      FlatRoom_Refresh(i);
    }
  }
  printf("*** [Database: Loaded] flat room data loaded (%d count)", cache_num_rows());
  return 1;
}
...
Flat_Refresh(flatid) {
  ...
  if (IsValidDynamic3DTextLabel(FlatData[flatid][flatGarageText])) {
    ...
    UpdateDynamic3DTextLabelText(FlatData[flatid][flatGarageText], COLOR_ARWIN, sprintf("[Flat Garage:%d]\n"YELLOW_E"%s",flatid,FlatData[flatid][flatName]));
  } else {
    FlatData[flatid][flatGarageText] = CreateDynamic3DTextLabel(sprintf("[Flat Garage:%d]\n"YELLOW_E"%s",flatid,FlatData[flatid][flatName]), COLOR_ARWIN, garagePosX, garagePosY, garagePosZ+0.5, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, World, Interior);
  }
  ...
  if (IsValidDynamic3DTextLabel(FlatData[flatid][flatText])) {
    ...
    UpdateDynamic3DTextLabelText(FlatData[flatid][flatText], COLOR_ARWIN, sprintf("[FB:%d]\n"YELLOW_E"%s\n"WHITE_E"Type: "YELLOW_E"%s\n{FFFFFF}Press '{FF0000}ENTER{FFFFFF}' to enter/exit",flatid,FlatData[flatid][flatName],type));
  } else {
    Flat_GetType(flatid, type);

    FlatData[flatid][flatText] = CreateDynamic3DTextLabel(sprintf("[FB:%d]\n"YELLOW_E"%s\n"WHITE_E"Type: "YELLOW_E"%s\n{FFFFFF}Press '{FF0000}ENTER{FFFFFF}' to enter/exit",flatid,FlatData[flatid][flatName],type), COLOR_ARWIN, PosX, PosY, PosZ+0.5, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, World, Interior);
  }
  return 1;
}
...
Flat_Create(name[], type, Float:x, Float:y, Float:z, world = 0, interior = 0) {
  ...
  if ((flatid = Iter_Free(Flat)) != cellmin) {
    ...
    mysql_tquery(g_SQL, sprintf("INSERT INTO `flat` (`Type`) VALUES ('%d');", FlatData[flatid][flatType]), "OnFlatCreated", "d", flatid);
    return flatid;
  }
  return cellmin;
}
...
Flat_Delete(flatid){
  ...
  mysql_tquery(g_SQL, sprintf("DELETE FROM `flat` WHERE `ID`='%d';",FlatData[flatid][flatID]));
...
Flat_Inside(playerid) {
  if (pData[playerid][pApartment] != -1){
    foreach (new flatid : Flat) if (FlatData[flatid][flatID] == pData[playerid][pApartment] && GetPlayerInterior(playerid) == FlatData[flatid][flatIntInterior] && GetPlayerVirtualWorld(playerid) == FlatData[flatid][flatIntWorld]) {
      return flatid;
    }
  }
  return -1;
...
FlatRoom_GetMaxCount(playerid) {
  new count = (pData[playerid][pVip] > 1 && pData[playerid][pVipTime]) ? (2) : (1);
  return count;
}
...
FlatRoom_Refresh(flatroom) {
  ...
  if (!FlatRoom[flatroom][flatRoomOwner]) {
    ...
    if (IsValidDynamic3DTextLabel(FlatRoom[flatroom][flatRoomText])) {
      ...
      UpdateDynamic3DTextLabelText(FlatRoom[flatroom][flatRoomText], COLOR_ARWIN, sprintf("[Flat:%d]\n"WHITE_E"Price: "GREEN_E"%s",flatroom,FormatMoney(FlatRoom[flatroom][flatRoomPrice])));
    } else {
      FlatRoom[flatroom][flatRoomText] = CreateDynamic3DTextLabel(sprintf("[Flat:%d]\n"WHITE_E"Price: "GREEN_E"%s",flatroom,FormatMoney(FlatRoom[flatroom][flatRoomPrice])), COLOR_ARWIN, PosX, PosY, PosZ, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, World, Interior);
    }
    ...
    if (FlatRoom[flatroom][flatRoomSeal]) {
      ...
      if (IsValidDynamic3DTextLabel(FlatRoom[flatroom][flatRoomText])){
        UpdateDynamic3DTextLabelText(FlatRoom[flatroom][flatRoomText], COLOR_ARWIN, sprintf("[Flat:%d]\n"WHITE_E"This flat is sealed by "RED_E"authority",flatroom,FlatRoom[flatroom][flatRoomOwner]));
      } else {
        FlatRoom[flatroom][flatRoomText] = CreateDynamic3DTextLabel(sprintf("[Flat:%d]\n"WHITE_E"This flat is sealed by "RED_E"authority"), COLOR_ARWIN, PosX, PosY, PosZ, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, World, Interior);
      }
    } else {
      if (IsValidDynamicPickup(FlatRoom[flatroom][flatRoomPickup]))
...
FlatRoom_Create(price, Float:x, Float:y, Float:z, world, interior, flatid) {
  ...
  if ((flatroom = Iter_Free(FlatRooms)) != cellmin) {
    ...
    mysql_tquery(g_SQL, sprintf("INSERT INTO `flatroom` (`Owner`) VALUES ('%d');", FlatRoom[flatroom][flatRoomOwner]), "OnFlatRoomCreated", "d", flatroom);
...
FlatRoom_Delete(flatroom) {
  ...
  mysql_tquery(g_SQL, sprintf("DELETE FROM `flatroom` WHERE `ID`='%d';",FlatRoom[flatroom][flatRoomID]));
...
FlatRoom_IsOwner(playerid, flatroom) {
  ...
  if (FlatRoom[flatroom][flatRoomOwner] != 0 && FlatRoom[flatroom][flatRoomOwner] == pData[playerid][pID])
    return 1;
...
FlatRoom_IsBuilder(playerid, flatroom) {
  ...
  if (FlatRoom[flatroom][flatRoomBuilder] != 0 && FlatRoom[flatroom][flatRoomBuilder] == pData[playerid][pID])
    return 1;
