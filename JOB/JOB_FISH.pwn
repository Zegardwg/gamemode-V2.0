//Fishing System -2158.5342,649.9276,52.3672
...
function FishTime(playerid)
{
	if(IsPlayerConnected(playerid) && pData[playerid][pInFish] == 1)
	{
	    ...
	    if(rand == 1)
	    {
	        ShowNotifInfo(playerid, "Anda mendapatkan sebuah sampah dan langsung membuangannya.", 10000);
		...
		else if(rand == 2)
		{
		    ...
		    pData[playerid][pWorm] -= 1;
	        pData[playerid][pInFish] = 0;
	        pData[playerid][pItuna] = weight;
			//Inventory_Add(playerid, "Ikan", 19630, weight);
	        TogglePlayerControllable(playerid, 1);
	        RemovePlayerAttachedObject(playerid, 9);
			return 1;
		}
		else if(rand == 3)
		{
		    ...
		    pData[playerid][pWorm] -= 1;
	        pData[playerid][pInFish] = 0;
	        pData[playerid][pItongkol] = weight;
			//Inventory_Add(playerid, "Ikan", 19630, weight);
	        TogglePlayerControllable(playerid, 1);
	        RemovePlayerAttachedObject(playerid, 9);
			return 1;
		...
		else if(rand == 5)
		{
		    ...
		    pData[playerid][pWorm] -= 1;
	        pData[playerid][pInFish] = 0;
	        pData[playerid][pIkembung] = weight;
			//Inventory_Add(playerid, "Ikan", 19630, weight);
	        TogglePlayerControllable(playerid, 1);
	        RemovePlayerAttachedObject(playerid, 9);
			return 1;
		...
		else if(rand == 6)
		{
		    ...
		    pData[playerid][pWorm] -= 1;
	        pData[playerid][pInFish] = 0;
	        pData[playerid][pImkarel] = weight;
			//Inventory_Add(playerid, "Ikan", 19630, weight);
	        TogglePlayerControllable(playerid, 1);
	        RemovePlayerAttachedObject(playerid, 9);
			return 1;
		...
		else if(rand == 7)
		{
		    ...
		    pData[playerid][pWorm] -= 1;
	        pData[playerid][pInFish] = 0;
	        pData[playerid][pItenggiri] = weight;
			//Inventory_Add(playerid, "Ikan", 19630, weight);
	        TogglePlayerControllable(playerid, 1);
	        RemovePlayerAttachedObject(playerid, 9);
			return 1;
		...
		else if(rand == 8)
		{
		    ...
		    pData[playerid][pWorm] -= 1;
	        pData[playerid][pInFish] = 0;
	        pData[playerid][pIBmarlin] = weight;
			//Inventory_Add(playerid, "Ikan", 19630, weight);
	        TogglePlayerControllable(playerid, 1);
	        RemovePlayerAttachedObject(playerid, 9);
			return 1;
...
CMD:jualikantuna(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pItuna] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan Tuna");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pItuna];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan tuna dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pFish] = 0;
	pData[playerid][pItuna] -= fish;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Tuna", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikantongkol(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pItongkol] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan tongkol");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pItongkol];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan tongkol dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pItongkol] -= fish;
	pData[playerid][pFish] = 0;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Tongkol", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikankakap(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pIkakap] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan kakap");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pIkakap];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan kakap dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pIkakap] -= fish;
	pData[playerid][pFish] = 0;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Kakap", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikankembung(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pIkembung] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan kembung");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pIkembung];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan kembung dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pIkembung] -= fish;
	pData[playerid][pFish] = 0;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Kembung", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikanmakarel(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pImkarel] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan Makarel");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pImkarel];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan Makarel dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pImkarel] -= fish;
	pData[playerid][pFish] = 0;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Makarel", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikantenggiri(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pItenggiri] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan Tenggiri");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pItenggiri];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan Tenggiri dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pItenggiri] -= fish;
	pData[playerid][pFish] = 0;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Tenggiri", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikanbmarlin(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pIBmarlin] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan Blue Marlin");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pIBmarlin];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan Blue Marlin dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pFish] = 0;
	pData[playerid][pIBmarlin] -= fish;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_BlueMarlin", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}

CMD:jualikanbsailfish(playerid, params[])
{
	if( pData[playerid][pProgress] == 1) return ErrorMsg(playerid, "Anda masih memiliki activity progress!");
	if( pData[playerid][pIsailF] < 1) return ErrorMsg(playerid, "Anda Tidak Memiliki Ikan Sail Fiss");

	ShowProgressbar(playerid, "Menjual Ikan..", 10);
	ApplyAnimation(playerid,"INT_HOUSE","wash_up",4.0, 1, 0, 0, 0, 0,1);
	new fish = pData[playerid][pIsailF];
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	Info(playerid, "Anda menjual semua ikan Sail Fiss dengan total uang "GREEN_E"%s", FormatMoney(pay));
	Food += fish;
	Server_MinMoney(pay);
	pData[playerid][pIsailF] -= fish;
	pData[playerid][pFish] = 0;
	new str[500];
	format(str, sizeof(str), "Received_%dx", pay);
	ShowItemBox(playerid, "Uang", str, 1212, 4);
	format(str, sizeof(str), "Removed_%dx", fish);
	ShowItemBox(playerid, "Ikan_Sail", str, 19630, 4);
	Inventory_Update(playerid);
	return 1;
}
...
/*CMD:sellfish(playerid, params[])
{
	...
	new fish = PlayerHasItem(playerid, "Ikan");
	new pay = fish * FishPrice;
	GivePlayerMoneyEx(playerid, pay);
	...
	Server_MinMoney(pay);
	pData[playerid][pFish] = 0;
	Inventory_Remove(playerid, "Ikan", fish);
	return 1;
}*/

/*CMD:selfish(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid, 3.0, 359.63, -2032.09, 7.83))
	{
		if(Inventory_Count(playerid, "Ikan") < 1)
			return Error(playerid, "You dont have fish.");

		if(Inventory_Count(playerid, "Ikan") > 45) 
			return ShowNotifError(playerid, "Anda sudah membawa Ikan!", 10000);
		
		new mstr[128];
		format(mstr, sizeof(mstr), ""WHITE_E"Masukan jumlah Ikan:\n"WHITE_E"Ikan Price"GREEN_E"%s /item", FormatMoney(FishPrice));
		ShowPlayerDialog(playerid, DIALOG_IKAN_SELL, DIALOG_STYLE_INPUT, "Sell Ikan", mstr, "Sell", "Cancel");
	}
	else return ShowNotifError(playerid, "Kamu tidak di penjualan atau pembelian Fish!", 10000);
	return 1;
}*/
